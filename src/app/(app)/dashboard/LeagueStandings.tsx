      ...
      <svg viewBox={`0 0 ${width} ${height}`} className="w-full" aria-label="League standings bars">
        ...
        {[0, 0.25, 0.5, 0.75, 1].map((p, i) => {
          ...
          return (
            <g key={i}>
              ...
              <text x={gx} y={height - 2} fontSize="10" fill="#6b7280" textAnchor="middle">{Math.round(p * maxPts)}</text>
            </g>
          );
        })}
        ...
        {teams.map((t, idx) => {
          ...
          return (
            <g key={t.teamId}>
              {/* Team label */}
              <text x={8} y={y + rowH / 2 + 4} fontSize="12" fill="#0f172a">{t.teamName}</text>
              {/* Bar background */}
              ...
              <text x={paddingLeft + barW - 6} y={y + rowH / 2} fontSize="12" fill="#fff" textAnchor="end" fontWeight={600} dominantBaseline="middle">{t.points}</text>
            </g>
          );
        })}
        ...
        {activeIdx !== null && (() => {
          ...
          const tipY = y + 4;
          return (
            <foreignObject x={tipX} y={tipY} width="210" height="72">
              <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 6, padding: 8, fontSize: 12, color: '#0f172a' }}>
